...
- **コンテンツ作成**: フェーズ（大単元）、モジュール（中単元）、ステップ（小単元）の階層構造で教材を作成。
  - **モジュール管理**: モジュールの作成、編集、削除が可能
    - 編集機能: 名前、説明、アイコン、カラー、表示順序を変更
    - 削除機能: モジュールと関連する全てのステップ、コンテンツ、問題を一括削除
...
- **生徒画面プレビュー**: 生徒用画面の見た目やコンテンツを教師権限で確認。
  - **ハイライト機能**: プレビュー画面でテキストや図形をハイライト可能。
    - テキスト選択: マウスで文字を選択してハイライト
    - 図形マーカー: グラフや画像上にマーカーを配置
    - カラーピッカー: ハイライト色を自由に変更可能
    - 削除機能: ハイライトをクリックして削除
...
- **学習画面 (/student)**: 割り当てられたセクションの学習コンテンツを閲覧。
  ...
  - **完了マーク**: 100%完了したフェーズとモジュールには緑色のチェックマークが表示
  - **復習機能**: 完了したモジュールには「復習」ボタンが表示され、いつでも復習可能
...
- **用語集 (/student/glossary)**: 登録された学習用語の検索と閲覧。
  - **統一されたデザイン**: ホーム画面と同じグレースケール基調
  - **検索機能**: 用語の絞り込み検索
- **モジュール完了判定**: 
  - 全てのステップの全ての問題に正解しないとモジュールを完了できません
  - 途中の問題をスキップして最後だけ解いても、完了できません
  - 解答履歴はデータベースに保存され、次回アクセス時も状態が保持されます
...
### 2026-02-03
- **モジュール編集・削除機能を実装**
  - 編集機能: モジュールの名前、説明、アイコン、カラー、表示順序を変更可能
  - 削除機能: カスケード削除により関連する全てのステップ、コンテンツ、問題を一括削除
  - `/teacher/modules` ページに編集・削除ボタンを追加
- **モジュール完了判定の仕様確認**
  - モジュール開始時に全問題と解答履歴をデータベースから読み込み
  - 各ステップ移動時に解答状況を保持（answeredQuestionsをクリアしない）
  - 最後のステップで「完了」ボタンを押す際、モジュール全体の未回答問題をチェック
  - 未回答があれば「○個の問題が未解答です」と警告を表示し、完了を阻止
  - 全ての解答履歴を`answer_history`テーブルに保存し、次回アクセス時も状態を復元
  - これにより、途中の問題をスキップして最後だけ解いてもモジュール完了できないことを保証
- **完了マークと復習機能を追加**
  - フェーズとモジュールに完了状態を視覚的に表示（緑色のチェックマーク）
  - 完了したモジュールに「復習」ボタンを追加
  - `/api/student/module-completion` エンドポイントでモジュール完了状態を取得
  - 進捗バーの色が完了時に緑色に変化
